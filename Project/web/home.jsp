...
<html lang="vi">
        ...
        <c:if test="${not empty listNews}">
            <section class="py-4">
                <div class="container">
                    ...
                    <h3 class="section-title font-weight-bold">Tất cả bài viết</h3>
    ...
    <section class="py-4">
        <div class="container">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3 class="section-title font-weight-bold mb-0">Tất cả bài viết</h3>
                <c:if test="${listNews.size() > 4}">
                    <div class="d-flex gap-2">
                        <button class="btn btn-sm btn-outline-light news-scroll-prev" style="border-radius: 50%; width: 36px; height: 36px; display: inline-flex; align-items: center; justify-content: center;">
                            <i class="fas fa-chevron-left"></i>
                        </button>
                        <button class="btn btn-sm btn-outline-light news-scroll-next" style="border-radius: 50%; width: 36px; height: 36px; display: inline-flex; align-items: center; justify-content: center;">
            ...
            <div class="news-scroll-wrapper" style="position: relative; overflow-x: auto; overflow-y: hidden; white-space: nowrap; scroll-behavior: smooth; -webkit-overflow-scrolling: touch; padding-bottom: 10px;">
                <div class="d-inline-flex gap-3" style="white-space: normal;">
                    <c:forEach items="${listNews}" var="n">
                        <div class="news-item" style="width: 280px; flex-shrink: 0; margin-right: 20px; display: inline-block; white-space: normal;">
                            <div class="card product-card" style="height: auto; border-radius: 15px; transition: all 0.3s ease;">
                                <div class="position-relative">
                                    <img src="${n.image}" class="card-img-top" alt="${n.title}"
                                         style="height: 160px; width: 100%; object-fit: cover; border-radius: 15px 15px 0 0;"
                                         onerror="this.src='https://via.placeholder.com/280x160?text=No+Image'">
                                    <c:if test="${not empty n.storeId}">
                                        <span class="badge badge-warning position-absolute p-1 px-2" style="top: 10px; right: 10px; font-size: 0.7rem; background: var(--primary);">Shop News</span>
                                    </c:if>
                                </div>
                                <div class="card-body p-3">
                                    <h6 class="font-weight-bold text-white text-truncate mb-1">${n.title}</h6>
                                    <p class="small text-muted text-truncate mb-2" style="display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${n.content}</p>
                                    <div class="d-flex justify-content-between align-items-center mt-2">
                                        <small class="text-muted">
                                            <fmt:formatDate value="${n.createdAt}" pattern="dd/MM/yyyy"/>
                                        </small>
                                        <a href="newsDetail?id=${n.id}" class="btn btn-link p-0 text-warning small">
    ...
    <style>
        ...
        /* Ẩn nút cuộn trên mobile nếu cần */
        @media (max-width: 576px) {
            .news-scroll-prev, .news-scroll-next {
    ...
    <script>
        $(document).ready(function() {
            const $scrollWrapper = $('.news-scroll-wrapper');
            const $prevBtn = $('.news-scroll-prev');
            const $nextBtn = $('.news-scroll-next');

            function checkButtons() {
                if (!$prevBtn.length || !$nextBtn.length) return;

                const scrollLeft = $scrollWrapper.scrollLeft();
                const maxScroll = $scrollWrapper[0].scrollWidth - $scrollWrapper[0].clientWidth;
            ...
            $scrollWrapper.on('scroll', checkButtons);
